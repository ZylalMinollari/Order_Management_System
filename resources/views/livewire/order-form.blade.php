                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form wire:submit.prevent="save">
                        ...
                        <div>
                            ...
                            <x-select2 class="mt-1" id="customer" name="customer" :options="$this->listsForFields['users']" wire:model="order.user_id" />
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="submit" wire:submit.prevent="save">
                        <div>
                            <x-input-label class="mb-1" for="customer" :value="__('Customer')" />
                            <x-select2 class="mb-" id="customer" name="user" :options="$this->listForFields['users']"> </x-select2>
                            <x-input-error :messages="$errors->get('orders.user_id')" class="mt-2" />
                        </div>
                        <div class="mt-4">
                            <x-input-label class="mb-1" for="order_date" :value="__('Order date')" />
                            <input x-data x-init="new Pikaday({ field: $el, format: 'MM/DD/YYYY' })" type="text" placeholder="MM/DD/YYYY"
                                id="order_date"
                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                            <x-input-error :messages="$errors->get('order.order_date')" class="mt-2" />
                        </div>
                        <table class="mt-4 min-w-full border divide-y divide-gray-200">
                            <thead>
                                <th class="px-6 py-3 text-left bg-gray-100">
                                    <span class="text-xs font-medium tracking-wider leading-4 text-gray-400 uppercase">
                                        Product
                                    </span>
                                </th>
                                ...
                                <th class="px-6 py-3 text-left bg-gray-100">
                                    <span
                                        class="text-xs font-medium tracking-wider leading-4 text-gray-400 uppercase">Quantity</span>
                                </th>
                                <th class="px-6 py-3 w-56 text-left bg-gray-100"></th>
                            </thead>
                            <tbody>
                                @forelse ($orderProducts as $index=>$orderProduct )
                                    <tr>
                                        <td
                                            class="px-6 py-4 text-sm leading-5 text-gray-900 whitespace-no-wrap dark:bg-gray-50">
                                            @if ($orderProduct['is_saved'])
                                                <input type="hidden"
                                        ...
                                        <td
                                            class="px-6 py-4 text-sm leading-5 text-gray-900 whitespace-nowrap dark:bg-gray-50">
                                            @if ($orderProduct['is_saved'])
                                                <input type="hidden"
                                        ...
                                        <td
                                            class="px-6 py-4 text-sm leading-5 text-gray-900 whitespace-nowrap dark:bg-gray-50">
                                            @if ($orderProduct['is_saved'])
                                                <x-primary-button class="dark:bg-gray-900 dark:text-white"
                        ...
                        <div class="mt-3">
                            <x-primary-button wire:click.prevent="addProduct">
                                + Add Product
                            </x-primary-button>
                        </div>

                        <div class="justify justify-end">
                            <table>
                                <tr>
